...
class Validator
{
    ...
    {
        //var_dump($this->getPart($this->_fields, explode('.', $params[0])));
    ...
    /**
     * Get a string containing the list of generated validation errors.
    ...
    public function errorsToString()
    {
        ...
        if (isset($this->_errors)) {
            foreach ($this->_errors as $key => $error) {
                if (is_array($error)) {
                    foreach ($error as $msg_key => $message) {
    ...
    public function errorsToJson()
    {
        $error_messages = '';
        if (isset($this->_errors)) {
            $error_messages = json_encode($this->_errors);
        }
        return $error_messages;
    }
